    ...
    p1_horizons = pd.DataFrame(
        {
            ...
            "hzname": ["A", "Bt1", "Bt2", "C"],
            "clay": [15, 35, 40, 20],
            "color": ["#A0522D", "#8B4513", "#8B4513", "#D2B48C"],
        }
    )
...
def test_glom(sample_spc):
    ...
    glom_dom_name = sample_spc.glom(
        intervals=standard_intervals, v="hzname", agg_fun="dominant", output="dataframe"
    )
    glom_dom_name_wide = glom_dom_name.pivot(
        index=["top", "bottom"], columns="id", values="hzname"
    ).reset_index()

    assert glom_dom_name_wide.shape == (4, 5)
    assert glom_dom_name_wide.loc[0, "P1"] == "A"

    glom_dom_clay = sample_spc.glom(
        intervals=standard_intervals, v="clay", agg_fun="dominant", output="dataframe"
    )
    glom_dom_clay_wide = glom_dom_clay.pivot(
        index=["top", "bottom"], columns="id", values="clay"
    ).reset_index()
...
def test_from_dataframe():
    ...
    schema = {"profile_id": "id", "h_id": "hzid", "d_top": "top", "d_bottom": "bottom"}
